...
## This module contains the core memory/data types for the AST from the parser,
## it's part of an attempt to migrate the compiler towards a set of lowerings.
## Presently, it's focused on bridging the AST produced by the legacy compiler
## into a newer AST.
##
## Significant AST changes are not possible as sufficient backwards
## compatibility is required in order to migrate.
##
## Naming Conventions:
## * `legacy` related to bridging from old AST to the new one

from ".." / ast import TNodeKind, PNode, safeLen, `[]`, pairs
from data_token import TokenIndex, `==`
...
type
  ...
  AstNode* = object
    ...
    left*: AstLeft
      ## xxx: need to find a way to hide this again
...
const
  assumedAstLen*: Natural = 100
    ## until we have a heurisitic this is a "reasonable" ast reservation to
...
proc initAstNode*(kind: AstNodeKind, token: TokenIndex): AstNode {.inline.} =
  ## create an AST node with only `kind` and `token` initialized.
  AstNode(
    kind: kind,
    token: token,
    left: emptyAstLeft,
...
proc `==`*(a, b: AstIndex): bool {.borrow.}
proc `==`*(a, b: AstLeft): bool {.borrow.}
proc `==`*(a, b: AstRight): bool {.borrow.}

func legacyNodeToAstKind*(n: PNode): AstNodeKind =
  ## map a `PNode`'s to an AstNodeKind, this is determined primarily based on
  ## the `TNode.kind` field, but occasionally looks at child count
  let
    kind = n.kind
...
proc reserveAstNode*(
  ...
  ast.nodes.add initAstNode(kind, token)

  result = AstIndex ast.nodes.len - 1

func isUninitialized(a: Ast; idx: AstIndex): bool =
  ## whether the given position has an existing AstNode or is it set to zero
  ## values as if freshly zero'd out memory.
  doAssert idx.uint32 < a.nodes.len.uint32, "idx is out of range: " & $idx
  let
    node = a.nodes[idx.uint32]
