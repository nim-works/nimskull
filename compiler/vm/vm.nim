...
proc rawExecute(c: var TCtx, pc: var int, tos: var StackFrameIndex): RegisterIndex =
  ...
  while true:
    ...
    of opcNDynBindSym:
      ...
      # IDEA: instead of calling back into ``sem`` via a VM callback registered
      #       inside ``semDynamicBindSym`` (which has the severe problem of
      #       capturing a ``PContext``, keeping it alive until the ``TCtx``
      #       instance is destroyed), we could yield back control to ``sem``
      #       with the information that a symbol lookup should be performed.
      #       The lookup result is then stored into a register also
      #       communicated earlier and VM execution is resumed.
      #       In some way (but not exactly), this is similar to returning a
      #       single ``nnkIdent`` node from a macro, and then passing the
      #       resulting semantically analysed AST back into another macro as
      #       a ``typed`` argument.
      #       But there is also the question if ``semDynamicBindSym`` is
      #       something worth keeping, as it allows AST generated by macros to
      #       forego scoping and symbol visibility rules (the normal
      #       ``bindSym`` allows for it too, albeit to a lesser degree).
