...
block conversions_for_non_ast_varargs:
  ...
  block multiple_differing:
    ...
    calledCheck 3:
      before(expected, "test", 1, -3'i8)

  block as_array:
    # expectation is that the argument must properly semcheck, so the
    # conversion doesn't really help, we can't pass in `[1, "test"]`. On the
    # plus side this is good because all arguments sem check consistently, on
    # the downside, perhaps treating `[...]` as syntax and moving away from
    # varargs initially being an openarray is a good thing? this is likely a
    # deep discussion
    let expected = ["1", "test"]
    calledCheck 2:
