discard """
description: '''
  . From https://github.com/nim-lang/Nim/issues/9410
    Refs and ptrs aren't properly stored in object fields and in arrays on JS target
  . Are pointers even supposed to work on the JS target?
  . The presence of tests such as taddr.nim in the js category suggests otherwise.
    From what I gathered the jsgen impl does try to handle ptrs and refs
    (including non object refs) already, but it's just quite unfinished and
    buggy right now.
  . The codegen can support it and it's part of Nim. It's the compiler's job
    to bridge over these differences and we should try to minimize them.
    It's not unsolvable.
'''
"""
template tests =
