...
proc tryExec*(db: DbConn, query: SqlQuery, args: varargs[string, `$`]): bool {.
  ...
  return mysql.realQuery(PMySQL db, q.cstring, q.len) == 0'i32

proc rawExec(db: DbConn, query: SqlQuery, args: varargs[string, `$`]) =
  var q = dbFormat(query, args)
  if mysql.realQuery(PMySQL db, q.cstring, q.len) != 0'i32: dbError(db)
...
proc tryInsertId*(db: DbConn, query: SqlQuery,
  ...
  var q = dbFormat(query, args)
  if mysql.realQuery(PMySQL db, q.cstring, q.len) != 0'i32:
    result = -1'i64
  else:
