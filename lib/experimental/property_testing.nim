...
## 
## Users define `Property`(s) that specify a constraint that must hold for a
## given set of inputs. These properties are then tested against a number of
## randomly generated inputs, which are generated by `Arbitrary`(s), that
## produce structured data matching the input types using `Random` inputs.
## `Arbitrary`(s) can be composed to produce complex data structures, beyond
...
import std/[
    macros, 
    mersenne, 
    options, # required for counter examples
  ]
...
type
  ...
  # TODO: rework to be a random byte stream generator
  Random* = object
  ...
  # TODO: rework to support hypothesis style shrinking, see
  #       https://hypothesis.works/articles/integrated-shrinking/ and
...
# TODO: add more tuple arbitraries
